...
val_want_Ez=find(value_Ez <frequecy_check & value_Ez > -frequecy_check);
value_Ez_show=value_Ez(value_Ez <frequecy_check & value_Ez > -frequecy_check);
...
%     grid on;
    u_2_der_i1_1=a_i__1*vec_Ez(N/2-1,val_want_Ez(k_cnt))+a_i_0*vec_Ez(N/2,val_want_Ez(k_cnt))+a_i_1*vec_Ez(N/2+1,val_want_Ez(k_cnt))+a_i_2*vec_Ez(N/2+2,val_want_Ez(k_cnt));
    u_2_der_i2_1=a_i1__2*vec_Ez(N/2-1,val_want_Ez(k_cnt))+a_i1__1*vec_Ez(N/2,val_want_Ez(k_cnt))+a_i1_0*vec_Ez(N/2+1,val_want_Ez(k_cnt))+a_i1_1*vec_Ez(N/2+2,val_want_Ez(k_cnt));
   
    u_2_der_j1_2=a_j__1*vec_Ez(N/2+M-1,val_want_Ez(k_cnt))+a_j_0*vec_Ez(N/2+M,val_want_Ez(k_cnt))+a_j_1*vec_Ez(N/2+M+1,val_want_Ez(k_cnt))+a_j_2*vec_Ez(N/2+M+2,val_want_Ez(k_cnt));
    u_2_der_j2_2=a_j1__2*vec_Ez(N/2+M-1,val_want_Ez(k_cnt))+a_j1__1*vec_Ez(N/2+M,val_want_Ez(k_cnt))+a_j1_0*vec_Ez(N/2+M+1,val_want_Ez(k_cnt))+a_j1_1*vec_Ez(N/2+M+2,val_want_Ez(k_cnt));
    
    criterion(i,1)=epi_minus*[(vec_Ez(N/2,val_want_Ez(k_cnt))-vec_Ez(N/2-1,val_want_Ez(k_cnt)))/h+(u_2_der_i1_1/h)];
    criterion(i,2)=epi_plus*[(vec_Ez(N/2+2,val_want_Ez(k_cnt))-vec_Ez(N/2+1,val_want_Ez(k_cnt)))/h-(u_2_der_i2_1/h)];
    criterion(i,3)=epi_plus*[(vec_Ez(N/2+M,val_want_Ez(k_cnt))-vec_Ez(N/2+M-1,val_want_Ez(k_cnt)))/h+(u_2_der_j1_2/h)];
    criterion(i,4)=epi_minus*[(vec_Ez(N/2+M+2,val_want_Ez(k_cnt))-vec_Ez(N/2+M+1,val_want_Ez(k_cnt)))/h-(u_2_der_j2_2/h)];
    k_cnt=k_cnt+1;
end
for i=1:length(val_want_Ez)
    ...
    conv(z,1)=criterion_divide(1,1);
    conv(z,2)=criterion_divide(1,2);
z=z+1;
end
toc;